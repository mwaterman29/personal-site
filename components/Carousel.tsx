	...
	return (
		<div className='relative flex grow-0 max-w-full max-h-full overflow-hidden'>
			<div className='flex items-center justify-center w-full h-full'>
				<img className='object-contain items-center justify-center rounded-md' alt='Preview Image' src={urls[index]} />
			</div>
			{urls.length > 1 && (
							...
							{urls.map((_, _i) =>
							{
								if (index == _i)
								{
									return (
										<button
											key={`indicator-${_i}`}
											className='transition-all ease-in-out w-10 bg-gray-300 text-gray-500 h-2'
											onClick={() =>
											{
												setIndex(_i);
											}}
								...
								return (
									<button
										key={`indicator-${_i}`}
										className='transition-all ease-in-out w-4 bg-gray-500 text-gray-500 h-2'
										onClick={() =>
										{
											setIndex(_i);
										}}
